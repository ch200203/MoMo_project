...
function pValue() {
	document.getElementsByName("clubName")[0].value = window.opener.document.reviewClub.clubName.value; 
 }

function findClubName() {
	var name = document.getElementsByName("clubName")[0].value;
	...
	httpRequest.open("POST", "momo.do?command=findClubName", true);
	...
	if (httpRequest.readyState === XMLHttpRequest.DONE) {
		if (httpRequest.status === 200) {
    		...
    		if (resultText) {
    			...
    			var trTags = '';
    			var clubList = jsonObj.clubList;

                //td 생성
                clubList.forEach(function(val){
                	trTags += '<tr><td>' + val.clubName + '</td>';
                	trTags += '<td><input type="button" class="btn"';
                	trTags += ' onclick="sendCheckValue(\'' + val.clubName + '\')" value="선택"></td>';
                	trTags += '</tr>';
                });
        	...
        	alert('모임명 찾기에 문제가 있어요.');
		}
	}
}

function sendCheckValue(clubName){
	// 회원가입 화면의 ID입력란에 값을 전달
	opener.document.reviewClub.clubName.value = clubName;
...
<body style="background: lavender" onload="pValue()">
	<div id="wrap">
		...
		<br>
		<center>
			<b><font size="4" color="#7643ea">모임명 찾기</font></b> <br>
			...
			<div id="chk">
				<div id="checkForm">
					...
					<input type="text" name="clubName" id="clubName"
						...
						type="button" value="찾기" class="btn" onclick="findClubName()">
